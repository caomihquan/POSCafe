...
public sealed class PaymentOutboxPublisher(IServiceScopeFactory scopeFactory, IProducer<string, string> producer, IOptions<OutboxOptions> options, ILogger<PaymentOutboxPublisher> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception exception) { logger.LogError(exception, "Payment outbox publisher failed"); }
    ...
    private async Task PublishBatchAsync(CancellationToken cancellationToken)
    {
        ...
        {
            try { using var activity = MessagingTelemetry.ActivitySource.StartActivity("messaging.publish", System.Diagnostics.ActivityKind.Producer); activity?.SetTag("messaging.system", "kafka"); activity?.SetTag("messaging.destination.name", options.Value.Topic); activity?.SetTag("messaging.message.id", message.Id); activity?.SetTag("messaging.destination.partition.key", message.AggregateId); var schemaId = IntegrationSchemaIds.ForEventType(message.EventType); await producer.ProduceAsync(options.Value.Topic, new Message<string, string> { Key = message.AggregateId, Value = message.Payload, Headers = new Headers { { "event-type", System.Text.Encoding.UTF8.GetBytes(message.EventType) }, { "event-id", System.Text.Encoding.UTF8.GetBytes(message.Id.ToString()) }, { "correlation-id", System.Text.Encoding.UTF8.GetBytes(message.CorrelationId ?? message.Id.ToString("N")) }, { "causation-id", System.Text.Encoding.UTF8.GetBytes(message.Id.ToString()) }, { "traceparent", System.Text.Encoding.UTF8.GetBytes(activity?.Id ?? string.Empty) }, { "schema-version", System.Text.Encoding.UTF8.GetBytes("1") }, { "schema-id", System.Text.Encoding.UTF8.GetBytes(schemaId) } } }, cancellationToken); message.ProcessedOnUtc = DateTime.UtcNow; message.LockedUntilUtc = null; message.Error = null; MessagingMetrics.Published.Add(1, new KeyValuePair<string, object?>("service", "payment")); }
            catch (Exception exception)
            {
                ...
                if (reachedLimit && !string.IsNullOrWhiteSpace(options.Value.DeadLetterTopic))
                {
                    ...
                    {
                        logger.LogError(deadLetterException, "Failed to publish payment outbox message {MessageId} to DLQ; retaining retry lease", message.Id);
                ...
                MessagingMetrics.PublishFailures.Add(1, new KeyValuePair<string, object?>("service", "payment"));
                logger.LogWarning(exception, "Failed to publish payment outbox message {MessageId}", message.Id);
            }
        }
        await db.SaveChangesAsync(cancellationToken);
    }
